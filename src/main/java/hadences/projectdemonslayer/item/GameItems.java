...
public class GameItems {
    ...
    ItemStack Ability2;
    ItemStack Ability3;
    ItemStack Katana = new ItemStack(Material.WOODEN_SWORD);
    ItemMeta meta;
    ArrayList<Component> lore = new ArrayList<>();

    public void givePlayerGameItems(Player p, int CustomModelData) {
        ...
        for (String form : ds.getConfig().getConfigurationSection("Ability." + BREATHING.getName() + ".Forms").getKeys(false)) {
            if ((Integer) ds.getConfig().get("Ability." + BREATHING.getName() + ".Forms." + form + ".Classification") == playerdata.get(p.getUniqueId()).getABILITY1()) {
                ...
                addDescription(lore, Chat.format((String) ds.getConfig().get("Ability." + BREATHING.getName() + ".Forms." + form + ".Description")));
                meta.lore(lore);
        ...
        giveKatana(p, 3, CustomModelData);
    }

    public void giveKatana(Player p, int slot, int CustomModelData) {
        //Katana
        ...
        lore.add(Component.text(Chat.format("&7―――――――「 &fINFO &7」―――――――")));
        lore.add(Component.text(Chat.format(" ")));
        lore.add(Component.text(Chat.format("&6▸ &f[&3RC&f] &6Ability Skill: &fSlash")));
        lore.add(Component.text(Chat.format("&4▸ Ability Damage: &f" + ds.getConfig().get("Universal.Skills.Slash.Damage"))));
        lore.add(Component.text(Chat.format("&6▸ Stamina Cost: &f" + ds.getConfig().get("Universal.Skills.Slash.StaminaCost"))));
        lore.add(Component.text(Chat.format("&3▸ Cooldown : &f" + ds.getConfig().get("Universal.Skills.Slash.Cooldown"))));
        lore.add(Component.text(Chat.format(" ")));
        lore.add(Component.text(Chat.format("&7―――――「 &fDESCRIPTION &7」―――――")));
        addDescription(lore, Chat.format("&7Special blades used by and made specifically for the Demon Slayer Corps to slay Demons. &aPlace it onto your offhand to move faster!"));
        meta.lore(lore);
        meta.setLocalizedName("katana");
        meta.setCustomModelData(CustomModelData);
        Katana.setItemMeta(meta);
        p.getInventory().setItem(slot, Katana);

    }

    public ItemStack getKatana(int CustomModelData) {
        //Katana
        meta = Katana.getItemMeta();
        // meta.addEnchant(Enchantment.DAMAGE_ALL,2,true);
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
